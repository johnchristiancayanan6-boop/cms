...
```bash
docker exec -i eastwest_db psql -U $POSTGRES_USER -d eastwest -c "UPDATE DATABASECHANGELOGLOCK SET LOCKED=false, LOCKGRANTED=null, LOCKEDBY=null WHERE ID=1;"
```
...
```bash
docker exec -i eastwest_db psql -U $POSTGRES_USER -d eastwest -c "SELECT * FROM DATABASECHANGELOGLOCK;"
```
