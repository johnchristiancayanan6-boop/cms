...
show_usage() {
    echo "Usage: ./eastwest-backup.sh [backup|restore] [options]"
    echo ""
    echo "Commands:"
    echo "  backup                Create a backup of Eastwest CMMS"
    echo "  restore <filename>    Restore Eastwest CMMS from a backup file"
    echo ""
    echo "Options:"
...
backup_eastwest_cmms() {
    BACKUP_FILENAME="eastwest_backup_${TIMESTAMP}.tar.gz"
    TEMP_DIR="/tmp/eastwest_backup_$TIMESTAMP"

    echo "Starting Eastwest CMMS backup..."
    ...
    if [ "$SKIP_DB" != "true" ]; then
        ...
        if ! check_container_running "eastwest_db"; then
            echo "Error: Eastwest database container (eastwest_db) is not running."
            echo "Make sure your Eastwest CMMS environment is up and running."
            rm -rf "$TEMP_DIR"
            exit 1
        fi

        docker exec eastwest_db pg_dump -U "$POSTGRES_USER" --encoding=UTF8 eastwest > "$TEMP_DIR/eastwest_db.sql"
    ...
    if [ "$SKIP_FILES" != "true" ]; then
        ...
        if ! check_container_running "eastwest_minio"; then
            echo "Error: MinIO container (eastwest_minio) is not running."
            echo "Make sure your Eastwest CMMS environment is up and running."
            rm -rf "$TEMP_DIR"
            exit 1
        fi
...
restore_eastwest_cmms() {
    ...
    TEMP_DIR="/tmp/eastwest_restore_$TIMESTAMP"

    echo "Starting Eastwest CMMS restore..."
    ...
    if [ "$SKIP_DB" != "true" ] && [ -f "$TEMP_DIR/eastwest_db.sql" ]; then
        ...
        if [[ "$confirm" =~ ^[yY] ]]; then
            ...
            # Restore from backup
            docker exec -i eastwest_db psql -U "$POSTGRES_USER" -d eastwest --set client_encoding=UTF8 < "$TEMP_DIR/eastwest_db.sql"
...
case "$COMMAND" in
    backup)
        ...
        ;;
    restore)
        restore_eastwest_cmms "$BACKUP_FILE"
        ;;
    help)
        show_usage
        ;;
