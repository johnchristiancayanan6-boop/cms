# Eastwest CMMS Backup and Restore Utility
# PowerShell version for Windows with Docker Desktop
# This script allows users to backup and restore the Eastwest CMMS system
...
function Show-Usage {
    Write-Host "Usage: .\eastwest-backup.ps1 [backup|restore] [options]"
    Write-Host ""
    Write-Host "Commands:"
    Write-Host "  backup                Create a backup of Eastwest CMMS"
    Write-Host "  restore <filename>    Restore Eastwest CMMS from a backup file"
    Write-Host ""
    Write-Host "Options:"
...
function Backup-EastwestCMMS {
    $BACKUP_FILENAME = "eastwest_backup_${TIMESTAMP}.zip"
    $TEMP_DIR = Join-Path $env:TEMP "eastwest_backup_$TIMESTAMP"

    Write-Host "Starting Eastwest CMMS backup..."
    ...
    if (-not $SkipDb) {
        ...
        if (-not (Test-ContainerRunning -ContainerName "eastwest_db")) {
            Write-Host "Error: Eastwest database container (eastwest_db) is not running." -ForegroundColor Red
            Write-Host "Make sure your Eastwest CMMS environment is up and running."
            Remove-Item -Path $TEMP_DIR -Recurse -Force -ErrorAction SilentlyContinue
            exit 1
        }

        $dbBackupPath = Join-Path $TEMP_DIR "eastwest_db.sql"
        docker exec eastwest_db bash -c "export PGCLIENTENCODING='UTF8'; pg_dump -U $POSTGRES_USER eastwest" > $dbBackupPath
    ...
    if (-not $SkipFiles) {
        ...
        if (-not (Test-ContainerRunning -ContainerName "eastwest_minio")) {
            Write-Host "Error: MinIO container (eastwest_minio) is not running." -ForegroundColor Red
            Write-Host "Make sure your Eastwest CMMS environment is up and running."
            Remove-Item -Path $TEMP_DIR -Recurse -Force -ErrorAction SilentlyContinue
            exit 1
        }
...
chmod +x /usr/bin/mc
mc alias set eastwest-minio http://minio:9000 "$MINIO_USER" "$MINIO_PASSWORD" --api S3v4
mc mirror eastwest-minio/eastwest-bucket /backup_data
...
function Restore-EastwestCMMS {
    ...
    $TEMP_DIR = Join-Path $env:TEMP "eastwest_restore_$TIMESTAMP"

    Write-Host "Starting Eastwest CMMS restore..."
    ...
    if ((-not $SkipDb) -and (Test-Path (Join-Path $TEMP_DIR "eastwest_db.sql"))) {
        ...
        if ($confirm -match "^[yY]") {
            ...
            docker exec eastwest_db bash -c "PGPASSWORD=$POSTGRES_PWD psql -U $POSTGRES_USER -d postgres -c 'DROP DATABASE IF EXISTS eastwest_old;'"

            # Rename current "eastwest" to "eastwest_old"
            docker exec eastwest_db bash -c "PGPASSWORD=$POSTGRES_PWD psql -U $POSTGRES_USER -d postgres -c 'ALTER DATABASE eastwest RENAME TO eastwest_old;'"

            # Create new "eastwest" database
            docker exec eastwest_db bash -c "PGPASSWORD=$POSTGRES_PWD psql -U $POSTGRES_USER -d postgres -c 'CREATE DATABASE eastwest;'"

            Get-Content $dbBackupPath | docker exec -i eastwest_db bash -c "export PGCLIENTENCODING='UTF8'; psql -U $POSTGRES_USER -d eastwest"
...
chmod +x /usr/bin/mc
mc alias set eastwest-minio http://minio:9000 "$MINIO_USER" "$MINIO_PASSWORD" --api S3v4
mc mirror /backup_data eastwest-minio/eastwest-bucket --overwrite
...
switch ($Command) {
    ...
    "restore" {
        Restore-EastwestCMMS -BackupFile $BackupFile
    }
    "help" {
        Show-Usage
    }
